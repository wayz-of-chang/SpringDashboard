...
app.controller('DashboardController', [ "$scope", "$mdDialog", "service", function($scope, $mdDialog, service) {
    var dashboard = this;
    dashboard.current = "1";
    ...
    dashboard.open_new_popup = function() {
        $mdDialog.show({contentElement: '#new_dashboard_modal', parent: angular.element(document.body)});
    };
    dashboard.create_new_dashboard = function() {
        ...
        service.create_dashboard(data, function(response) {
            if (response.status >= 200 && response.status < 300) {
                ...
                setTimeout(function() { dashboard.new_dashboard.success = false; $mdDialog.hide(); }, 3000);
            } else {
    ...
    dashboard.open_edit_popup = function() {
        $mdDialog.show({contentElement: '#edit_dashboard_modal', parent: angular.element(document.body)});
    };
    dashboard.update = function() {
        ...
        service.edit_dashboard(data, function(response) {
            if (response.status >= 200 && response.status < 300) {
                ...
                dashboard.select(response.data.data.id);
                setTimeout(function() { dashboard.edit_dashboard.success = false; $mdDialog.hide(); }, 3000);
            } else {
    ...
    dashboard.confirm_delete_popup = function() {
        $mdDialog.show({contentElement: '#delete_dashboard_modal', parent: angular.element(document.body)});
    };
